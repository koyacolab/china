...
class MODISExporter:
    ...
    def __init__(
        self,
        locations_filepath=Path("data/china_yield/wheat_2002-2018.csv"),    # locations_filepath=Path("data/yield_data.csv")
        collection_id="MODIS/051/MCD12Q1",
    ):
        self.locations = load(locations_filepath)

        print('_init_ : MODISExporter :', self.locations.columns)
    ...
    def export(
        ...
        # cwd = os.getcwd()
        # download_folder = Path( str(os.path.join(cwd, 'data', folder_name)) )
        # print('pwwd : ', cwd, folder_name, download_folder)
        # fn
        ...
        # print('exp img : ', type(img)) 
        # print('exp img. : ', type(img), img.getInfo())
        ...
        # print('exp region COUNTYFP : ', type(region))

        count = 0

        # print('MAJOR_STATES :', MAJOR_STATES)
        # Generate Feature Collection with China Countys from yield.file    
        features=[]
        ...
        for state_id, county_id in np.unique(
            ...
            # print(type(file_region), type(processed_img),type(file_region))
            # fn
            file_region = None
            while True:
                try:
