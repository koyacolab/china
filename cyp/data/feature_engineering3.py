...
class Engineer3:
    ...
    def __init__(
        self,
        cleaned_data_path=None,  # Path("data/img_output"),
        yield_data_filepath=None,  # Path("data/china_yield/wheat_2002-2018.csv"),
        county_data_filepath=None, #Path("data/china_yield/county_data.csv"),
    ):
        ...
        yield_data = load(yield_data_filepath)[
            ["Year", "State", "County", "Value", "SownArea", "YieldVal"]
        ]
        yield_data.columns = ["Year", "State", "County", "Value", "SownArea", "YieldVal"]
        yield_data['County'] = pd.to_numeric(yield_data['County'], downcast='integer')
        ...
        # dfff = county_data.loc[ county_data['County'] == 0 ]
        # print(dfff.columns)
        # print(dfff.head(30)[['County', 'Longitude']])

        self.yield_data = pd.merge( yield_data, county_data, on=["County", "State"] )
        # self.yield_data['County'] = pd.to_numeric(self.yield_data['County'], downcast='integer')
        # print(self.yield_data.columns)
        # print(self.yield_data.head(30)[["County", "State", "Year"]])
        # dfff = county_data.loc[ self.yield_data['County'] == 0 ]
        # print('dfff : ', dfff.columns)
        # print(dfff.head(30)[['Year', 'County']])
        # fn
    ...
    def _calculate_histogram(
        ...
        for im in np.split(imagecol, imagecol.shape[-1] / bands, axis=-1):
            ...
            for i in range(im.shape[-1]):
                density, _ = np.histogram(im[:, :, i], bin_seq[i], density=False)
                # print('density : ', type(density), density.shape, density)
                # print('im max : ', np.max(im[:,:,i]))
    ...
    def process(
        ...
        # print(self.yield_data.head()[['County', 'Year']])

        # self.yield_data[ int(self.yield_data['Year']) < YEARS ]
        ...
        for yield_data in self.yield_data.itertuples():
            ...
            sownarea = yield_data.SownArea
            yieldval = yield_data.YieldVal
            # area = yield_data.Area
            # yieldos = yield_data.Yield

            # print('process : ', year, county, state)
            ...
            if filename in self.processed_files:
                ...
                if generate == "mean":
                    ...
                    # print('histogram : ', filename, type(image), image.shape)
                    ...
                    # print('histogram 2 : ', type(image), image.shape)
                    # fig, axs = plt.subplots(1, 1, figsize =(10, 7), tight_layout = True)
                    # axs.hist(image[0,0,:], bins = num_bins)
                    # # Show plot
                    # plt.show()
                    # fin
                ...
                years.append(year)
                sownareas.append(sownarea)
                yieldvals.append(yieldval)
        ...
        np.savez(
            self.cleaned_data
            / f'corn_histogram_all_{"mean" if (generate == "mean") else "full"}.npz',
            ...
            output_sownareas=np.array(sownareas),
            output_yieldvals=np.array(yieldvals),
        )
