...
class DataCleaner:
    ...
    def __init__(
        self,
        mask_path=None,  #Path("data/crop_yield-data_mask"),
        temperature_path=None,  #Path("data/crop_yield-data_temperature"),
        image_path=None,  #Path("data/crop_yield-data_image"),
        yield_data_path=None,  #Path("data/china_yield/wheat_2002-2018.csv"),
        savedir=None,  #Path("data/img_output"),
        ...
        print(os.path.join('.', self.image_path))
        ...
        self.start_year = int(self.TASK['RANGE_DOWNLOAD'][0].split('-')[0])
        print('start year : ', type(self.start_year), self.start_year)

        self.num_years = self.TASK['NUM_PROC_YEARS']
    ...
    def process(self, delete_when_done=False, checkpoint=True):
        """
        ...
        if not self.multiprocessing:
            print('Multiprocessing None : ', self.tif_files)
            ...
            start_year_iter = repeat(self.start_year)
            num_years_iter = repeat(self.num_years)
...
def process_county(
    ...
    #     print(checkpoint, num_years)
    #     print(list(savedir.glob(f"*_{state}_{county}.npy"))[0])
...
def divide_into_years(img, bands, composite_period, num_years=None, extend=False):
    """
    Parameters
    ----------
...
def merge_image_lists(im_list_1, num_bands_1, im_list_2, num_bands_2):
    ...
    for im1, im2 in zip(im_list_1, im_list_2):
        ...
        print('Merge : ', type(im1), im1.shape, type(im2), im2.shape)
        icount = icount + 1
