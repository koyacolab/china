...
class ModelBase:
    ...
    def _run_1_year(
        ...
        # print('model_information[key] : ', model_information.keys())
        # fn
    ...
    def _train(
        ...
        scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[900,1250,1500,1750], gamma=0.1)
        ...
        for epoch in range(num_epochs):
            ...
            TB_writer.add_scalar("Loss - L1_train", np.array(running_train_scores['l1']).mean(), epoch)
            TB_writer.add_scalar("Loss - L2_train", np.array(running_train_scores['l2']).mean(), epoch)
            TB_writer.add_scalar("Loss - loss_train", np.array(running_train_scores['loss']).mean(), epoch)
            TB_writer.add_scalar("Loss - L1/L2_train", l1vl2_train, epoch)

            TB_writer.add_scalar("Loss - L1_valid", np.array(running_val_scores['l1']).mean(), epoch)
            TB_writer.add_scalar("Loss - L2_valid", np.array(running_val_scores['l2']).mean(), epoch)
            TB_writer.add_scalar("Loss - loss_valid", np.array(running_val_scores['loss']).mean(), epoch)
            TB_writer.add_scalar("Loss - L1/L2_valid", l1vl2_val, epoch)
