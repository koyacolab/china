...
class ModelBase2:
    ...
    def run(
        ...
        print('years : ', years)
        print('output_image : ', type(images))
        fn
    ...
    def _train(
        ...
        for epoch in range(num_epochs):
            ...
            pprint("TRAINING: {}, {}".format(", ".join(train_output_strings), l1vl2_train))
            pprint("VALIDATION: {}, {}".format(", ".join(val_output_strings), l1vl2_val))
            ...
            if len(EarlyStopping_train) > 100: 
              ES_train = round( sum(EarlyStopping_train[epoch-20:-1])/20, 3 )
              ES_val = round( sum(EarlyStopping_val[epoch-20:-1])/20, 3)
              pprint('Step:{}, Epoch:{}, LR:{}, ES_train:{}, ES_val:{}'.format(step_number, epoch, scheduler.get_last_lr(), ES_train, ES_val))
              if ES_train < 0.4:
                print('Early Stopping in epoch: {}'.format(epoch)) 
                break

            TB_writer.add_scalar("Loss/train", l1vl2_train, epoch)
